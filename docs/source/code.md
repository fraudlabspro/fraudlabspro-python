...
```{py:function} get_transaction(params)
Retrieve an existing transaction from FraudLabs Pro fraud detection system.

:param dict params: (Required) The parameters of FraudLabs Pro transaction detail.
...
| fraudlabspro_rules                            | list   | Return the rules triggered by the system. Available on Mini plan onward only.  |
...
:param dictionary params: (Required) The details of a transaction.
...
| fraudlabspro_score                            | integer | Overall score between 1 and 100. 100 is the highest risk and 1 is the lowest risk. |
| fraudlabspro_status                           | string  | Final action based on the rules analysis. Return values: APPROVE \| REJECT \| REVIEW |
| fraudlabspro_rules                            | dictionary   | Return the rules triggered by the system. Available on Mini plan onward only. |
...
```{py:function} verify_sms(id, type)
